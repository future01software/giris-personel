...
async def login(credentials: UserLogin):
    ...
    if DEMO_MODE and (identity == "admin" or identity == "admin@gatekeeper.com") and credentials.password == "admin123":
        user = {
            "id": "demo_admin",
            "username": "admin",
    ...
    # Try finding by identity (could be username or email)
    user = await db.users.find_one({"username": identity}, {"_id": 0})
    if not user:
