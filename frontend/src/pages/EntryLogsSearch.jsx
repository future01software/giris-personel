...
const formatDurationSec = (sec) => {
  ...
  const s = Number(sec);
  if (!Number.isFinite(s) || s <= 0) return '<1 dk';

  const totalMin = Math.floor(s / 60);
  if (totalMin <= 0) return '<1 dk';
...
const gateText = (gate) => (gate ? (GATE_LABELS[gate] || gate) : '');
...
const EntryLogsSearch = () => {
  ...
  }, [q, ioFilter, useDayFilter, day, fetchDayTotalsForSelectedDay]);

  const hint = useMemo(() => {
    if (useDayFilter) return 'Gün filtresi açık: sadece seçili gün taranır.';
    return 'Gün filtresi kapalı: geriye dönük tüm kayıtlar taranır.';
  }, [useDayFilter]);
      ...
      <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-100 dark:border-slate-700 shadow-sm p-5">
        <div className="flex flex-col gap-4">
          ...
          <input
            ...
            placeholder="İsim / şirket / not / görevli / saha ara..."
            className="w-full px-4 py-3 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg text-slate-900 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-blue-500"
          />
          ...
          <div className="flex items-center gap-3">
            ...
            <input
              ...
              className="px-3 py-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg text-slate-900 dark:text-slate-100 text-sm disabled:opacity-60"
            />
          </div>
          ...
          <div className="flex flex-wrap gap-2">
            <button
              ...
              className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${ioFilter === 'all'
                ...
                : 'bg-slate-50 dark:bg-slate-900 text-slate-700 dark:text-slate-300 border border-slate-200 dark:border-slate-700 hover:bg-slate-100 dark:hover:bg-slate-800'
                }`}
            >
              Tümü
            </button>
          ...
          {!!info.scannedPages && (
            <div className="text-xs text-slate-600 dark:text-slate-300 bg-slate-50 dark:bg-slate-900 rounded-lg px-3 py-2 border border-slate-200 dark:border-slate-700">
              Taranan sayfa: <b>{info.scannedPages}</b> / {info.totalPages} — Bulunan: <b>{info.found}</b>
              {useDayFilter && (
                <>
                  {' '}— Toplam süre: {dayTotalsLoading ? <b>hesaplanıyor...</b> : <b>hazır</b>}
                </>
              )}
            </div>
          )}
      ...
      <div className="space-y-3">
        {loading ? (
          <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-100 dark:border-slate-700 p-12 text-center">
            ...
            <p className="text-slate-500 dark:text-slate-400">Aranıyor...</p>
          </div>
        ) : results.length === 0 ? (
          <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-100 dark:border-slate-700 p-12 text-center">
          ...
          results.map((log) => {
            ...
            return (
              <div
                key={log.id}
                className="bg-white dark:bg-slate-800 rounded-xl border border-slate-100 dark:border-slate-700 shadow-sm hover:shadow-md transition-shadow p-4"
              >
                <div className="flex items-start justify-between gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {(log._gate_label || log._gate || log.gate || log.site || log.location) ? (
                      <p className="text-xs font-semibold text-indigo-600 dark:text-indigo-400 mt-1">
                        📍 {log._gate_label || gateText(log._gate || log.gate || log.site || log.location || '')}
                      </p>
                    ) : null}
                  ...
                  <div className="flex flex-col gap-1.5 text-xs min-w-[200px]">
                    {useDayFilter && !isIn && (
                      <div className="flex items-center justify-between">
                        <span className="text-slate-500 dark:text-slate-400">Toplam Süre:</span>
                        <span className="font-bold text-slate-900 dark:text-white">
                          {dayTotalsLoading ? '...' : formatDurationSec(totalSec)}
                        </span>
                      </div>
                    )}
                    <div className="flex items-center justify-between pt-1.5 border-t border-slate-100 dark:border-slate-700">
                      <span className="text-slate-500 dark:text-slate-400">Güvenlik:</span>
